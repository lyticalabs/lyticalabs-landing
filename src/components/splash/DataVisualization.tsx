    ...
    <div className="relative w-full h-96 flex items-center justify-center">
      ...
      <div className="relative w-80 h-80">
        ...
        {[0, 1, 2, 3, 4, 5].map((index) => {
          // Use static positions for SSR, dynamic for client
          const angle = isClient ? (index * 60 + animationProgress * 2) * (Math.PI / 180) : index * 60 * (Math.PI / 180);
          const radius = isClient ? 100 + Math.sin(animationProgress * 0.05 + index) * 20 : 100;
        ...
        <div className="absolute -top-8 -right-8 z-30">
          <div className="bg-gray-900/80 backdrop-blur-sm border border-green-400/30 rounded-lg p-3 min-w-32">
            <div className="flex items-end gap-1 h-12">
              {dataPoints.slice(0, 8).map((point, index) => (
                <div
                  key={index}
                  className="bg-gradient-to-t from-green-600 to-green-400 rounded-sm transition-all duration-500 flex-1"
        ...
        <div className="absolute -bottom-8 -left-8 z-30">
          <div className="bg-gray-900/80 backdrop-blur-sm border border-green-400/30 rounded-lg p-3">
            <div className="text-2xl font-bold text-green-400">
              {isClient ? Math.floor(85 + Math.sin(animationProgress * 0.1) * 10) : 85}%
            </div>
        ...
        <div className="absolute -bottom-8 -right-8 z-30">
          <div className="bg-gray-900/80 backdrop-blur-sm border border-green-400/30 rounded-lg p-3 flex flex-col items-center justify-center min-h-24">
            <div className="relative w-16 h-16 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 32 32">
                ...
                <circle
                  ...
                  strokeWidth="4"
                  strokeLinecap="round"
                  strokeDasharray={`${isClient ? Math.sin(animationProgress * 0.05) * 10 + 15 : 15} 75.4`}
                ...
                <circle
                  ...
                  strokeWidth="4"
                  strokeLinecap="round"
                  strokeDasharray={`${isClient ? Math.cos(animationProgress * 0.07) * 8 + 12 : 12} 75.4`}
                  strokeDashoffset="-25"
                  className="transition-all duration-500"
                />
                <circle
                  ...
                  strokeWidth="4"
                  strokeLinecap="round"
                  strokeDasharray={`${isClient ? Math.sin(animationProgress * 0.03 + 1) * 6 + 8 : 8} 75.4`}
                  strokeDashoffset="-45"
                  className="transition-all duration-500"
                />
        ...
        <svg className="absolute inset-0 w-full h-full transform -rotate-90 z-10" viewBox="0 0 100 100">
          ...
          <circle
            ...
            strokeWidth="0.5"
            strokeLinecap="round"
            strokeDasharray={`${isClient ? (animationProgress * 2.83) : 0} 283`}
