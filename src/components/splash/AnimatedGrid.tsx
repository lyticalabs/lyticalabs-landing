...
export function AnimatedGrid() {
  // When the user prefers reduced motion, we skip the JS tick entirely and
  // render a static grid. CSS animations are also disabled via the global
  // reduced-motion block in `globals.css`, so the whole surface goes quiet.
  ...
  return (
    <div className="absolute inset-0 w-full h-full min-h-screen opacity-10 pointer-events-none">
      {/* Main Grid Pattern */}
      <svg className="w-full h-full" style={{ background: 'transparent' }}>
        <defs>
          ...
          <pattern id="dots" width="30" height="30" patternUnits="userSpaceOnUse">
            <circle cx="15" cy="15" r="1" fill="rgb(16 185 129 / 0.15)" />
          </pattern>
      ...
      <div className="absolute inset-0">
        ...
        {[20, 40, 60, 80].map((top, index) => (
          <div
            key={`h-${index}`}
            className="absolute w-full h-px bg-gradient-to-r from-transparent via-emerald-400/15 to-transparent animate-flow-horizontal"
            style={{
              top: `${top}%`,
        ...
        {[15, 35, 55, 75, 85].map((left, index) => (
          <div
            key={`v-${index}`}
            className="absolute w-px h-full bg-gradient-to-b from-transparent via-emerald-400/15 to-transparent animate-flow-vertical"
            style={{
              left: `${left}%`,
            ...
            return (
              <div
                ...
                className={`absolute w-2 h-2 rounded-full transition-all duration-500 ${
                  isActive
                    ? 'bg-emerald-400 shadow-lg shadow-emerald-400/50 scale-150'
      ...
      <div className="absolute top-10 left-10">
        <svg width="200" height="100" className="opacity-30">
          ...
          />
          <circle cx="50" cy="50" r="3" fill="rgb(16 185 129)" className="animate-ping" />
          <circle
            cx="150"
            cy="80"
            r="3"
